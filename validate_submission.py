...
def check_hf_space(space_url: str) -> bool:
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Space URL returned {response.status_code}")
            return False
        ...
        if reset_response.status_code != 200:
            print(f"❌ Reset endpoint returned {reset_response.status_code}")
            return False
        ...
        print(f"❌ HF Space check failed: {e}")
        return False
...
def check_openenv_compliance() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"❌ OpenEnv validation failed: {result.stderr}")
            return False
            
    except Exception as e:
        print(f"❌ OpenEnv validation error: {e}")
        return False
...
def check_inference_script() -> bool:
    ...
    for var in required_vars:
        if var not in content:
            print(f"❌ Required variable {var} not found in inference.py")
            return False
    
    # Check OpenAI client usage
    if 'from openai import OpenAI' not in content:
        print("❌ Must use OpenAI client for LLM calls")
        return False
    ...
    for pattern in stdout_patterns:
        if pattern not in content:
            print(f"❌ Required STDOUT pattern {pattern} not found")
            return False
...
def check_tasks_and_graders() -> bool:
    ...
    for func in task_functions:
        if func not in grader_content:
            print(f"❌ Grader function {func} not found")
            return False
...
def check_dockerfile() -> bool:
    ...
    if '7860' not in dockerfile_content:
        print("❌ Dockerfile must expose port 7860 for HF Spaces")
        return False
